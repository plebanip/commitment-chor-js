...
import {
  ...
  heightOfTopBands,
  hasBandMarker
} from 'chor-js/lib/util/BandUtil';
import { MESSAGE_DISTANCE } from 'chor-js/lib/util/MessageUtil';
 ...
 */
export default function CommitmentChoreoRenderer(config, eventBus, textRenderer, pathMap) {
  ...
  this.drawCommitment = function(p, element){
    console.log(p)
    console.log(element)
    element.height = 80;
    element.width = 100
    ...
    svgAttr(shape, {
      d: getTaskOutline(
          0, 0, element.width, element.height,0
      ),
      fill: 'white',
      fillOpacity:  0.95,
      stroke: 'black',
  ...
  this.drawChoreographyActivity = function(p, element) {
    console.log("Drawing choreography activity");
    // Draw the outer stroke and background
    let shape = svgCreate('path');
    svgAttr(shape, {
      d: getTaskOutline(
...
inherits(CommitmentChoreoRenderer, BaseRenderer);

CommitmentChoreoRenderer.$inject = [
  'config',
  'eventBus',
  'textRenderer',
  'pathMap'
...
CommitmentChoreoRenderer.prototype.canRender = function(element) {
  console.log(element)
  return is(element, 'bpmn:ChoreographyActivity') ||
      is(element, 'bpmn:Participant') ||
      is(element, 'bpmn:Message') ||
      is(element, 'tc:Commitment');
};

CommitmentChoreoRenderer.prototype.drawShape = function(p, element) {
  ...
  }else if (is(element, 'tc:Commitment')) {
    return this.drawCommitment(p,element);
  }
};

CommitmentChoreoRenderer.prototype.getShapePath = function(shape) {
  console.log(shape)
  ...
  }else if (is(shape, 'tc:Commitment')){
    return getTaskOutline(shape.x,shape.y, shape.width,shape.height,1);
  }
};
